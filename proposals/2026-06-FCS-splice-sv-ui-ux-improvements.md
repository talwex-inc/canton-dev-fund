## Scope Amendment Proposal - SV governance UI/UX improvements
...
| Milestones | [#529](https://github.com/canton-foundation/canton-dev-fund/issues/529) [#552](https://github.com/canton-foundation/canton-dev-fund/issues/552) [#553](https://github.com/canton-foundation/canton-dev-fund/issues/553)
---

### Purpose

FCS submits this amendment to the Digital Asset Splice maintainer team for review and approval. It records additional work carried out under the approved grant beyond the scope of the in-scope issues as originally defined, and proposes a corresponding uplift.

---

### Basis

The Figma design references supplied by the Digital Asset design team formed the estimating and implementation basis for the affected issues. FCS scoped, estimated and implemented against those designs as supplied.

The designs were subsequently revised across multiple alignment cycles covering field order, component placement, labels and placeholder text. Bringing the delivered work in line with the revised designs required rebuilding work already completed against the original references. The revisions were not derivable from the designs as supplied.

Separately, three in-scope issues required backend investigation at maintainer request in order to identify the underlying problem. The approved proposal confines the grant to the `governance-beta` frontend and lists backend API changes beyond what individual issues require as an explicit non-goal, so this investigation sat outside the funded scope. It was carried out on request and delivered the findings the maintainer team needed: canton-network/splice#3453 was extended to backend investigation and changes, and the investigations under canton-network/splice#2451 and canton-network/splice#3025 established that the former was stale and that the latter was not deliverable as originally scoped due to backend incompatibility.

Where the revised designs required work falling outside the definition of an existing issue, new issues were raised in `canton-network/splice` so that the additional work is traceable in the repository rather than absorbed silently into existing PRs. These capture additional work only. The functional objective of the grant is unchanged, and FCS is not proposing new features or an expanded basket beyond delivering the agreed governance UI outcome.

This falls under the *Delivery and Acceptance Model* in the approved proposal, which covers review feedback that "materially changes accepted scope, introduces new requirements, or requires architectural rework beyond the original issue scope".

The additional cycles extended delivery of the affected work by approximately two weeks beyond the original milestone targets.

---

### Additional work

Only issues where the design revisions or the review findings above required work beyond the original definition are listed.

| Milestone | Issue | Additional work required | PR |
| :---- | :---- | :---- | :---- |
| M1 | canton-network/splice#2623, canton-network/splice#3023 | Multiple Figma-alignment cycles (field order, Show JSON placement, labels such as Threshold Deadline / Effective At, review surfaces). Confusion and late clarification on placeholder texts. | canton-network/splice#6410 |
| M3 | canton-network/splice#3453 | Backend investigation and, where possible, backend changes at maintainer request. | canton-network/splice#6458 |
| M3 | canton-network/splice#2451 | Out-of-scope backend investigation on request to identify the underlying problem, which established the issue was stale. | canton-network/splice#6470 |
| M3 | canton-network/splice#3025 | Out-of-scope backend investigation on request to identify the underlying problem, which established the original scope was not feasible due to backend incompatibility. | canton-network/splice#6491 |
| New | canton-network/splice#6910 | Changed scope: placeholder text | canton-network/splice#6972 |
| New | canton-network/splice#6912 | Changed scope: label/placeholder text | canton-network/splice#6972 |
| New | canton-network/splice#6914 | Changed scope: ID/URL spacing | canton-network/splice#6976 |
| New | canton-network/splice#6915 | Changed scope: scroll and width edges | canton-network/splice#6955 |

---

### Proposed uplift

FCS proposes an uplift of 10% of the approved grant, at the original reference rate of 1 CC = USD 0.15, for approval by the Digital Asset maintainer team.

| Approved #444 (merged 2026-07-03) | Proposed uplift (10%) |
| ----: | ----: | 
| 393,000 CC | 39,300 CC | 

The grant remains CC-denominated at the original reference rate. No adjustment is sought in respect of CC price movement. The volatility stipulation, acceptance criteria and delivery model in the approved proposal are unchanged.

Figma revision history, maintainer correspondence and PR history are available on request.
